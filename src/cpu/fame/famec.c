...
//    CCnt = io_cycle_counter;
	...
	#define READ_BYTE_PCREL(A, D)           \
		D = *(u8 *)(((A) ^ 1) + BasePC) & 0xFF;
//        D = m68000_read_pcrelative_8(A);

	#define READ_WORD_PCREL(A, D)           \
		D = *(u16 *)((A) + BasePC) & 0xFFFF;
//        D = m68000_read_pcrelative_16(A);

	#define READ_LONG_PCREL(A, D)               \
		D = (*(u16 *)((A) + BasePC) & 0xFFFF) << 16;         \
		D |= *(u16 *)((A) + BasePC + 2) & 0xFFFF;
//      D = (m68000_read_pcrelative_16(A) << 16) | m68000_read_pcrelative_16((A) + 2);

	#define READSX_WORD_PCREL(A, D)             \
		READ_WORD_PCREL(A, D);

	#define READSX_LONG_PCREL(A, D)             \
		READ_LONG_PCREL(A, D);
