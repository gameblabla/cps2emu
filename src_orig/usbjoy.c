  ...
  if (joy != NULL) {    
	...
	case JS_EVENT_AXIS:
	  if (events[i].number == 0) {
	    if (events[i].value == 0) joy->stateaxes[LEFT] = joy->stateaxes[RIGHT] = 0;
	    else if (events[i].value < 0) joy->stateaxes[LEFT] = 1;
	    else joy->stateaxes[RIGHT] = 1;
	  }
	  else if (events[i].number == 1) {
	    if (events[i].value == 0) joy->stateaxes[UP] = joy->stateaxes[DOWN] = 0;
	    else if (events[i].value < 0) joy->stateaxes[UP] = 1;
	    else joy->stateaxes[DOWN] = 1;
	  }
	  event = 1;
	  break;
...
int joy_getaxe (int axe, struct usbjoy * joy) {
  if (axe < 4 && joy != NULL) return joy->stateaxes[axe];
  else return -1;
}
